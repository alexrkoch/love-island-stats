...
if ( ! class_exists( 'Kadence_AMP_Get_Image' ) ) {
	
	class Kadence_AMP_Get_Image {
        /**
        ...
        public function process($id = null, $width = null, $height = null) {
			...
			if(empty($width) && empty($height) ) {
				return self::kt_get_full_image($id);
			}
		    ...
		    if ( self::kt_image_size_larger_than_original( $id, $width, $height ) ) {
		    	return self::kt_get_full_image($id);
			}
		    //Check if we have image.
		    if( self::kt_image_size_already_exists( $id, $width, $height ) ) {

				return self::kt_get_image_at_size($id, $width, $height );
		    	...
		    	if(800 < $width) {
		    		...
		    		if( $width == $height ) {
		    			...
		    			if(150 >= $width) {
		    				// Thumbnail
		    				return self::kt_get_image_at_size($id, 150,150);
		    			} else {
		    				return self::kt_get_full_image($id);
		    			}
		    		} else {
		    		...
		    		// Orginal ratio
		    		if(300 >= $width) {
		    			return self::kt_get_image_at_size_name($id, 'medium');
		    		} elseif(800 > $width) {
		    			return self::kt_get_image_at_size_name($id, 'large');
		    		} else {
		    			return self::kt_get_full_image($id);
		    		}
		    	}
		...
		public static function kt_get_image_srcset($id = null, $url = null, $width = null, $height = null) {
		...
		public static function kt_get_srcset_output($id = null, $url = null, $width = null, $height = null) {
		    $img_srcset = self::kt_get_image_srcset($id, $url, $width, $height);
		    if(!empty($img_srcset) ) {
		...
		public static function kt_image_size_larger_than_original($id, $width, $height) {
				$image_meta = self::kt_get_image_meta( $id );
		...
		public static function kt_get_full_image($id) {
				...
				$image = array (
					...
					1 => $src[1],
					2 => $src[2],
					3 => self::kt_get_srcset_output($id, $src[0], $src[1], $src[2]),
					4 => $src[0],
					5 => $id
				);
				return $image;
		}
		public static function kt_get_image_at_size($id, $width, $height) {
				$size = array(
					0 => $width,
					1 => $height
				);
		...
		public static function kt_get_image_at_size_name($id, $size) {
				$src = wp_get_attachment_image_src($id, $size );
				$full = wp_get_attachment_image_url($id, 'full' );
				// array return.
				$image = array (
		...
		public static function kt_image_size_already_exists( $id, $width, $height ) {
			$image_meta = self::kt_get_image_meta( $id );
